...
// const quantity = document.getElementById('quantity');
//
// plus.addEventListener('click', () => quantity.value = parseInt(quantity.value) + 1);
// minus.addEventListener('click', () => {
...
(function($) {
    ...
    // ✅ New Arrival Product Swiper JS
    document.addEventListener('DOMContentLoaded', function () {
        var $window = $(window);

        // Arrow position adjuster
        function adjustArrowPosition() {
            var $slideHeight = $(".collection-wrap .swiper-slide .product-image"),
                $buttonsHeight = $(".collection-wrap .swiper-buttons button");
        ...
        var swiper = new Swiper('#new-arrival-slider', {
            ...
            watchSlidesProgress: true,
            navigation: {
                prevEl: '.swiper-prev-new-arrival',
                nextEl: '.swiper-next-new-arrival'
            },
            pagination: {
...
(function(){
    ...
    const buyNowForm = document.getElementById('form-buy-now');

    if (!qtyInput || !buyNowQty || !buyNowForm) return;

    // Update hidden field when quantity changes
    qtyInput.addEventListener('input', function(){
        const v = parseInt(this.value) || 1;
        buyNowQty.value = v > 0 ? v : 1;
    });

    // On buy-now submit ensure value is synced (in case user changed via keyboard quickly)
...
$(document).on('click', '.quick-view', function () {
    ...
    modal.find('.new-price').text('৳' + data.price);
    modal.find('.available-stock').text(data.stock);
    modal.find('.sku-code').text(data.sku);
    modal.find('.product-desc').text(data.description);
        ...
        if (key.startsWith('image')) {
            imgHtml += `
                <div class="swiper-slide">
                    <img src="${data[key]}" class="w-100 img-fluid" alt="${data.name}">
                </div>`;
        }
    });

    // Insert image slides
    modal.find('#quickview-slider-big .swiper-wrapper').html(imgHtml);
    modal.find('#quickview-slider-small .swiper-wrapper').html(imgHtml);

    //  Re-initialize swiper
    if (typeof ST !== 'undefined' && typeof ST.quickView === 'function') {
        ST.quickView();
    }

    modal.find('.form-buy-now').attr('data-id', data.id);
    // modal.find('.form-buy-now').attr('href', 'http://127.0.0.1:8000' + data.id);
    modal.find('.form-buy-now').attr('href', 'http://127.0.0.1:8000/website/buy-now/' + data.id);
    modal.data('product-id', data.id);
