...
<script src="{{asset('/')}}website/assets/js/custome.js"></script>
            ...
            success:function(response){
                ...
                resultsHtml += `
                    <div class="p-3 text-center">
                        <a href="{{ route('products.all') }}?search=${searchText}" class="btn-style secondary-btn">
...
@if (Request::route()->getName() == 'website.buy-now')
    <script>
...
<script>
    ...
    $(document).on('show.bs.modal', '#quickview-modal', function (event) {
        ...
        // Update Links
        var buyNowUrl = "{{ route('website.buy-now', ['id' => 'PLACEHOLDER']) }}".replace('PLACEHOLDER', id);
        modal.find('#quick-view-buy-now').attr('href', buyNowUrl);

        var wishlistUrl = "{{ route('add-to-wishlist', ['id' => 'PLACEHOLDER']) }}".replace('PLACEHOLDER', id);
        modal.find('#quick-view-wishlist').attr('href', wishlistUrl);
        
        var productUrl = "{{ route('product', ['id' => 'PLACEHOLDER']) }}".replace('PLACEHOLDER', id);
